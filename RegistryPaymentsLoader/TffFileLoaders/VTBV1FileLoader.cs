...
{
    class VTBV1FileLoader : TffFileLoader
    {

        public VTBV1FileLoader()
        {
        }

        public override string Version => "VTBV1";

        protected override TffString LoadString(string[] tffStringParts)
        {
            if (tffStringParts.Length != 5) return null;
            return new VTBV1String(tffStringParts);
        }
        ...
        public override List<TffString> Load(Stream dataStream)
        {
            ...
            for (var j = 0; j < lines.Length; j++)
            {
                ...
                if (!Regex.IsMatch(sum, "^[0-9]+([.,][0-9]{1,2})?$") || 
                    ...
                    continue;
                tffStrings.Add(new VTBV1String(new string[] {
                    tenant,
                    address,
                    sum,
                    payDate
            ...
            if (NoticeDate != null)
            {
                ...
                {
                    (tffString as VTBV1String).NoticeDate = NoticeDate;
                }
            }
            return tffStrings;
        }
