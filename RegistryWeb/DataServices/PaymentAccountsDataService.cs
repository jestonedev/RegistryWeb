...
namespace RegistryWeb.DataServices
{
    public class PaymentAccountsDataService : ListDataService<PaymentsVM, PaymentsFilter>
    {
        ...
        internal void CreateClaimMassCustom()
        {
...
{68805,"01.04.2019" }
            ...
            var payments = GetPaymentsForMassReports(dict.Keys.ToList()).ToList();
            foreach (var payment in payments)
            {
                var beginDateStr = dict[payment.IdAccount];
                var beginDateParts = beginDateStr.Split(".");
                var beginDate = new DateTime(int.Parse(beginDateParts[2]), int.Parse(beginDateParts[1]), int.Parse(beginDateParts[0]));
                var claim = new Claim
                {
